...
if (isset($_POST['tambah'])) {
    ...
    $stmt = $conn->prepare(
        "INSERT INTO anggota_psi (kelompok, nama, nim) VALUES (?, ?, ?)"
    );
...
if (isset($_GET['hapus'])) {
    ...
    $stmt = $conn->prepare(
        "DELETE FROM anggota_psi WHERE id = ?"
    );
...
if (isset($_POST['edit'])) {
    ...
    $stmt = $conn->prepare(
        "UPDATE anggota_psi SET nama = ?, nim = ? WHERE id = ?"
    );
...
========================= */
$result = $conn->query(
    "SELECT * FROM anggota_psi ORDER BY kelompok ASC, created_at ASC"
);
?>
...
<head>
<meta charset="UTF-8">
<title>Perancangan Sistem Informasi | TI.23.C4</title>

<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">

<style>
body{
    background: linear-gradient(135deg, #0dcaf0, #6fe6f9);
...
<h2>Mata Kuliah Perancangan Sistem Informasi <br> Dosen Pengampu: Suherman, S.Kom., M.Kom. M.M</h2>
